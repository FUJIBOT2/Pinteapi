...
app.get('/search', async (req, res) => {
    ...
    if (!search) {
        return res.json({ error: 'Welcome to my API' });
    }
    ...
    const url = `https://www.pinterest.com/search/pins/?q=${search}&rs=typed&term_meta[]=${search}%7Ctyped`;
    ...
    try {
        ...
        if (arrMatch) {
            ...
            return res.json({ error: 'No matching data found' });
        }
    } catch (error) {
        console.error(error);
